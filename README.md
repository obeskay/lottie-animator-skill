...
**Generate, validate, and ship professional Lottie animations from SVGs**
...
> A practical motion-design skill for Claude Code: start from the SVG structure, choose a motion personality, build the smallest useful timeline, then validate the exported JSON before previewing it.
...
```bash
# 1. Open the live preview page (the new lottie is included)
